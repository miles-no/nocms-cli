...
module.exports = (context, container, runLocal) => {
  ...
  console.log(`     Starting ${container.isExternal ? 'external container ' : ''}${container.name} ${container.isExternal ? '' : (runLocal ? 'with local source code' : 'from registry')}...`); // eslint-disable-line
  if (!container) {
    console.error(`     ${chalk.red.bold('Invalid container name: ')}${container.name}`);
    return;
  }
  const { name, flags = [], ports, volumes = [] } = container;
